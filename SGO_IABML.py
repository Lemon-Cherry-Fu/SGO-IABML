...
class LSTM_model(nn.Module):
    def __init__(self, input_dim, hidden_dim):
        ...
        self.linear = torch.nn.Linear(self.hidden_dim, input_dim)

    def forward(self, grad, hx, cx):
        gates = self.lstm.weight_ih_l0 @ grad.T
        gates = gates + self.lstm.weight_hh_l0 @ hx.T
        gates = gates + self.lstm.bias_ih_l0[:, None] + self.lstm.bias_hh_l0[:, None]
        i_gate = torch.sigmoid(gates[:self.hidden_dim])
        f_gate = torch.sigmoid(gates[self.hidden_dim:2 * self.hidden_dim])

        output, states = self.lstm(grad, (hx, cx))

        return output, states, f_gate, i_gate
        
class LSTM_Optimizer(Optimizer):
    def __init__(self, params, lr, input_dim):
        self.lr = lr
        super(LSTM_Optimizer, self).__init__(params=params, defaults={})
        self.input_dim = input_dim
        self.hidden_dim = input_dim
        self.lstm_model = LSTM_model(input_dim=self.input_dim, hidden_dim=self.hidden_dim)
        self.hx = torch.zeros(1, self.hidden_dim)
        self.cx = torch.zeros(1, self.hidden_dim)
        self.vi_ = torch.zeros(1, self.input_dim)

        self.lstm_loss_fn = torch.nn.MSELoss(reduction="sum")
        self.lstm_optimizer = torch.optim.Adam(self.lstm_model.parameters(), lr=1e-3)

    def step(self, closure: Optional[Callable[[], float]] = None, train_optimizer=False) -> Optional[float]:
        if train_optimizer:
        ...
        for param_group in self.param_groups:
            ...
            output, (self.hx, self.cx), f_gate, i_gate = self.lstm_model(grad, self.hx, self.cx)

            vi = torch.mul(f_gate.T, self.vi_) + torch.mul(i_gate.T, grad)
            self.vi_ = vi.data


            vi_index = 0
            for param in params:
                data_num = param.data.numel()
                vi_reshape = vi[0][vi_index: vi_index + data_num]
                vi_reshape = vi_reshape.reshape(param.data.size())

                param.data = param.data - vi_reshape
                vi_index = vi_index + data_num
    ...
    def train_optimizer(self):
        ...
        for param_group in self.param_groups:
            ...
            grad = grad.unsqueeze(0)
            output, (self.hx, self.cx), f_gate, i_gate = self.lstm_model(grad, self.hx.data, self.cx.data)

            vi = torch.mul(f_gate.T, self.vi_) + torch.mul(i_gate.T, grad)

            lstm_loss = self.lstm_loss_fn(vi, self.vi_)
            # print(f'lstm_loss is {lstm_loss}')
            ...
            return lstm_loss.item()

    def save_lstm_model(self):
        torch.save(self.lstm_model.state_dict(), './model/lstm_model.pkl')

    def load_lstm_moder(self):
        self.lstm_model.load_state_dict(torch.load('./model/model_param.pkl'))

class SGO_IABML(MLBaseClass):
    """Implementation of SGO_IABML"""
    ...
    def load_model(
        ...
        model = dict.fromkeys(
            ('hyper_net', 'f_generator', 'f_base_net', 'optimizer', 'phi_hyper_net', 'f_phi_base_net', 'phi_optimizer'))
        ...
        model['optimizer'] = LSTM_Optimizer(
            params=model['hyper_net'].parameters(),
            lr=self.config['meta_lr'],
            input_dim=32645,
        )
    ...
    def train(
        ...
        try:
            for epoch_id in range(self.config['resume_epoch'], self.config['resume_epoch'] + self.config['num_epochs'],
                                  1):
                loss_monitor = []
                loss_phi_monitor = []
                ...
                tb_writer = SummaryWriter(
                    ...
                    purge_step=self.config['resume_epoch'] * self.config['num_episodes_per_epoch'] // self.config[
                        'minibatch_print'] if self.config['resume_epoch'] > 0 else None
                )
                for eps_count, eps_data in enumerate(train_dataloader):
                    ...
                    split_data = self.config['train_val_split_function'](eps_data=eps_data,
                    ...
                    if ((eps_count + 1) % self.config['minibatch'] == 0):
                        ...
                        model['optimizer'].step(train_optimizer=True)
                        lstm_loss = model['optimizer'].train_optimizer()
                        print(f'lstm_loss = {lstm_loss}')
                        ...
                        if (eps_count + 1) % self.config['minibatch_print'] == 0:
                            ...
                            global_step = (epoch_id * self.config['num_episodes_per_epoch'] + eps_count + 1) // \
                            ...
                            print(f'Loss/train is {np.mean(loss_monitor)}')
