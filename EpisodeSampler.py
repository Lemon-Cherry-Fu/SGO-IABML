...
class EpisodeSampler(torch.utils.data.BatchSampler):
    ...
    def __init__(self, sampler: torch.utils.data.Sampler[typing.List[int]], num_ways: int, num_samples_per_class: int,
    ...
    def __iter__(self) -> typing.Iterator[typing.List[int]]:
        while (True):
            ...
            for label in labels:
                batch.extend(
                    random.sample(population=self.class_img_idx[dataset_id][label], k=self.num_samples_per_class))
